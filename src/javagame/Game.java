...
public class Game extends BasicGameState {
	...
	private void restartCountDown(Graphics g) {
		if(waitCounter >= 0 && waitCounter < 1000) {
			g.drawString("restarting..", 365, 200);
			g.drawString("3", 400, 220);
		}
		else if(waitCounter >= 1000 && waitCounter < 2000) {
			g.drawString("restarting..", 365, 200);
			g.drawString("2", 400, 220);
		}
		else if(waitCounter >= 2000 && waitCounter < 3000) {
			g.drawString("restarting..", 365, 200);
			g.drawString("1", 400, 220);
		}
	}
	...
	public void spawnPawn() {
		if (Game.pawnTime <= System.currentTimeMillis() - Game.pawnStart) {
			...
			int randLoc = rand.nextInt(3);
			
			if (randLoc == 0) {
				addEnemy(new Pawn(diamond_enemy, rand.nextInt(800),
						0 - rand.nextInt(150)));
				Game.pawnStart = System.currentTimeMillis();
				Game.pawnTime = 200 + rand.nextInt(500);
			}
			else if (randLoc == 1) {
				addEnemy(new Pawn(diamond_enemy, rand.nextInt(800),
						600 + rand.nextInt(150)));
				Game.pawnStart = System.currentTimeMillis();
				Game.pawnTime = 200 + rand.nextInt(500);
			}
			else if (randLoc == 2) {
				addEnemy(new Pawn(diamond_enemy, 0 - rand.nextInt(150),
						rand.nextInt(600)));
				Game.pawnStart = System.currentTimeMillis();
				Game.pawnTime = 200 + rand.nextInt(500);
			}
	...
	public void spawnPinWheel() {
		if (Game.pinWheelTime <= System.currentTimeMillis()
				- Game.pinWheelStart) {
			...
			if (randLoc == 0) {
				if (randLoc2 == 0) {
					addEnemy(new PinWheel(pinwheel_enemy,
							450 + rand.nextInt(300), rand.nextInt(550)));
					Game.pinWheelStart = System.currentTimeMillis();
					Game.pinWheelTime = 800 + rand.nextInt(500);
				}
				if (randLoc2 == 1) {
					addEnemy(new PinWheel(pinwheel_enemy, rand.nextInt(750),
							350 + rand.nextInt(200)));
					Game.pinWheelStart = System.currentTimeMillis();
					Game.pinWheelTime = 800 + rand.nextInt(500);
				}
			}
			...
			if (randLoc == 1) {
				if (randLoc2 == 0) {
					addEnemy(new PinWheel(pinwheel_enemy, rand.nextInt(350),
							rand.nextInt(550)));
					Game.pinWheelStart = System.currentTimeMillis();
					Game.pinWheelTime = 800 + rand.nextInt(500);
				}
			...
			/*
			 * if(randLoc == 0) { addEnemy(new PinWheel(pinwheel_enemy, 400 +
			 * rand.nextInt(370), rand.nextInt(670))); Game.pinWheelStart =
			 * System.currentTimeMillis(); Game.pinWheelTime = 800 +
			 * rand.nextInt(500); } else if(randLoc == 1) { addEnemy(new
			 * PinWheel(pinwheel_enemy, rand.nextInt(375), rand.nextInt(670)));
			 * Game.pinWheelStart = System.currentTimeMillis();
			 * Game.pinWheelTime = 800 + rand.nextInt(500); }
			 */
		}
